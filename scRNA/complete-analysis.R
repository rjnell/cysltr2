...
for (data_dir in data_dirs) {
  ...
  expression_matrix = Read10X(data.dir = paste0("scrna/",data_dir))
  
  # Convert to Seurat object
  seurat_object = CreateSeuratObject(counts = expression_matrix, project = "Uveal melanoma", min.cells = 3, min.features = 100)
  
  # Save Seurat object
  saveRDS(um_data,paste0("scrna/",data_dir,"/",data_dir,".RDS"))
}

# Read all data
data = list()
for (data_dir in data_dirs) {
  data[[data_dir]] = readRDS(paste0("scrna/",data_dir,"/",data_dir,".RDS"))
}
...
saveRDS(um_data, "scRNA/um_data_complete.RDS")
...
um_data = readRDS("scRNA/um_data_complete.RDS")
...
um_data_cysltr2 = subset(um_data, cells = which(substr(colnames(um_data@assays$RNA@counts),1,6) %in% c("UMM059","UMM061","UMM062","UMM065","UMM064","UMM066","UMM069","UMM063")))

# Normalize data
um_data_cysltr2 = NormalizeData(um_data_cysltr2, normalization.method = "LogNormalize", scale.factor = 10000)
um_data_cysltr2 = FindVariableFeatures(um_data_cysltr2, selection.method = "vst", nfeatures = 2000)

# Scale data
all.genes = rownames(um_data_cysltr2)
um_data_cysltr2 = ScaleData(um_data_cysltr2, features = all.genes, vars.to.regress = c("nCount_RNA", "percent.mt"))

# Save scaled data
saveRDS(um_data_cysltr2, "scRNA/um_data_scaled.RDS")
